...
if(!is_dir($path)){
    ...
    var_dump(mkdirs($path2));
	//为上传的文件新起一个名字，保证更加安全
	$new_filename = "$id-$course_name-$date.jpg";
	//将文件从临时路径移动到磁盘
	if (move_uploaded_file($temp_name, "$path/".$new_filename)){
		if(copy("$path/$new_filename","$path2/$new_filename")){
		echo "<script>alert('假条上传成功！');window.history.go(-1);</script>";
		}
	}else{
		echo "<script>alert('假条上传失败,错误码：$error');</script>";
	}
