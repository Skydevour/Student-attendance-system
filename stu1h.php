...
$sql = "select * from student_course where student_id='$id' and time='1'" ;
...
if($input!=""){ 
	...
	$sql_1 = "select * from student_course where course_name='$input' and student_id='$id' " ;
...
function news($pageNum = 1, $pageSize = 3)
{
    ...
    $rs = "select * from student_course where student_id='$id' and time='1' limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
...
function allNews()
{
    ...
    mysqli_set_charset($coon, "utf8");
	$nowdate=date("Y-m-d");
    $rs = "select count(*) num from student_course where student_id='$id' and time='1' " ; //可以显示出总页数
    $r = mysqli_query($coon, $rs);
